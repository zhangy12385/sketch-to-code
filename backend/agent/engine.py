...
class AgentEngine:
    def __init__(
        ...
        # Created lazily in run() so we can wire the correct Gemini key
        # based on the actual model chosen for this variant.
        ...
        self.tool_runtime: AgentToolRuntime | None = None
    ...
    async def run(
        ...
        # Build the runtime now that we know which provider backs this
        # model. Only Gemini needs to be wired up as a separate credential
        # because extract_assets goes through the same SDK as the model.
        is_gemini = provider == "gemini"
        self.tool_runtime = AgentToolRuntime(
            file_state=self.file_state,
            **self._runtime_options,
            gemini_api_key=self.api_key if is_gemini else None,
            gemini_base_url=self.base_url if is_gemini else None,
        )
