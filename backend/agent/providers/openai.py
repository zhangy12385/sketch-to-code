...
class OpenAIProviderSession(ProviderSession):
    def __init__(
        ...
        self._prompt_report_logger = PromptReportLogger(
            ...
            api_model_name=api_model or get_openai_api_name(model),
        )
    ...
    def _resolved_api_model(self) -> str:
        return self._api_model or get_openai_api_name(self._model)
    ...
    async def stream_turn(self, on_event: EventSink) -> ProviderTurn:
        ...
        # Reasoning effort is bound to a specific Llm mapping; an operator
        # override on the model name means we let the API use its default
        # reasoning behavior rather than guessing.
