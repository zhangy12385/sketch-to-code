...
class AnthropicProviderSession(ProviderSession):
    def __init__(
        ...
        self._model = model
        self._tools = tools
        # The operator can override the model the SDK actually calls. Useful
        # for shadow-trafficing a new release or pinning a non-default build.
        self._api_model = api_model
        self._total_usage = TokenUsage()
        self._prompt_report_logger = PromptReportLogger(
            provider="anthropic",
            model=str(model.value),
            api_model_name=api_model or _get_anthropic_api_model_name(model),
        )
    ...
    def _resolved_api_model(self) -> str:
        return self._api_model or _get_anthropic_api_model_name(self._model)

    async def stream_turn(self, on_event: EventSink) -> ProviderTurn:
        stream_kwargs: Dict[str, Any] = {
            "model": self._resolved_api_model(),
            "max_tokens": 50000,
    ...
    async def close(self) -> None:
        ...
        pricing = MODEL_PRICING.get(self._resolved_api_model())
