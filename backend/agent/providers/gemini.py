...
class GeminiProviderSession(ProviderSession):
    def __init__(
        ...
        self._prompt_report_logger = PromptReportLogger(
            ...
            api_model_name=api_model or _get_gemini_api_model_name(model),
        )
    ...
    def _resolved_api_model(self) -> str:
        return self._api_model or _get_gemini_api_model_name(self._model)

    async def stream_turn(self, on_event: EventSink) -> ProviderTurn:
        api_model_name = self._resolved_api_model()
        # Thinking levels are tied to a specific Llm mapping. If the operator
        # overrode the model id, drop the level so the API uses its default
        # instead of guessing.
        thinking_level = (
            _get_thinking_level_for_model(self._model)
