...
async def extract_assets_from_images(
    ...
    # The google-genai SDK exposes a custom endpoint through HttpOptions.
    # When the operator doesn't override the URL, keep the default behavior
    # so the SDK still uses its built-in Generative Language API base.
    if gemini_base_url:
        client = genai.Client(
