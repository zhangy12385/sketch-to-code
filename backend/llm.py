...
class Llm(Enum):
    ...
    # Kimi / Moonshot coding endpoint (api.kimi.com/coding)
...
KIMI_MODELS = {m for m, p in MODEL_PROVIDER.items() if p == "kimi"}
...
# Per-provider groupings, also exposed for callers that need a quick
# "is this model an OpenAI / Anthropic / Gemini / Kimi" check without
# re-deriving it from MODEL_PROVIDER.
...
def get_provider_for_model(model: Llm) -> str:
