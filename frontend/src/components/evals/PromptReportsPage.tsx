  ...
  return (
    <div className="min-w-0">
      ...
      >
        {expanded
          ? "收起"
          : `展开全部 ${text.length.toLocaleString()} 个字符`}
      </button>
    </div>
  );
}
...
function UsagePanel({ usage }: { usage: PromptReportUsage }) {
  const metrics: [string, string][] = [
    ["输入 tokens", usage.input_tokens.toLocaleString()],
    ["输出 tokens", usage.output_tokens.toLocaleString()],
    ["缓存读取", usage.cache_read.toLocaleString()],
    ["缓存写入", usage.cache_write.toLocaleString()],
    ["总 tokens", usage.total_tokens.toLocaleString()],
    ["缓存命中率", `${usage.cache_hit_rate_percent.toFixed(1)}%`],
    [
      "费用",
      usage.cost_usd === null ? "无" : `$${usage.cost_usd.toFixed(4)}`,
    ],
  ];
...
function PromptReportsPage() {
  ...
  const handlePrune = async () => {
    if (
      !window.confirm(
        "删除所有超过 7 天的报告？这也会一并移除遗留的 run_logs 文件。"
      )
    ) {
      return;
    }
    ...
    try {
      ...
      const data = await response.json();
      toast.success(
        `已清理 ${data.deleted_count} 项${data.deleted_count === 1 ? "" : ""}，释放 ${formatBytes(data.freed_bytes)}。`,
        { duration: 8000 }
      );
        ...
        <div className="grid min-h-0 flex-1 gap-4 lg:grid-cols-[360px_minmax(0,1fr)]">
          <section className="min-h-0 overflow-y-auto rounded-2xl border border-zinc-800 bg-zinc-900/60 p-2">
            <div className="px-2 py-1 text-xs uppercase tracking-wide text-zinc-500">
              {isLoadingList
                ? "加载中…"
                : `${sessionGroups.length} 个请求${
                    sessionGroups.length === 1 ? "" : ""
                  } · ${reports.length} 个回合${reports.length === 1 ? "" : ""}`}
            </div>
